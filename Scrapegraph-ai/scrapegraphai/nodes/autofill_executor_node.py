# scrapegraphai/nodes/autofill_executor_node.py
...
# ------------------------ DEFAULT (START-URL) HANDLER ------------------------ #
@router.default_handler
async def default_handler(context: PlaywrightCrawlingContext) -> None:
    """
    Handles the start URL and enqueues category links.
    {{ META_COMMENTS }}
    (Optional: LLM can insert extra notes here.)
    """
    context.log.info(f'default_handler is processing {context.request.url}')

    # {{ WAIT_FOR_DEFAULT_SELECTOR }}
    # Example: await context.page.wait_for_selector('div.category')

    # {{ ENQUEUE_DEFAULT_LINKS }}
    # Example:
...
async def category_handler(context: PlaywrightCrawlingContext) -> None:
    """
    Handles category pages, queues product/detail links, and paginates.
    """
    context.log.info(f'category_handler is processing {context.request.url}')

    # {{ WAIT_FOR_CATEGORY_SELECTOR }}
    # Example: await context.page.wait_for_selector('.item-card')

    # {{ ENQUEUE_CATEGORY_LINKS }}
    # Example:
    # await context.enqueue_links(
    #     selector='.item-card > a',
    #     label='DETAIL',
    # )

    # {{ PAGINATION_LOGIC }}
    # Example:
    # next_btn = await context.page.query_selector('a.next')
    # if next_btn:
    #     await context.enqueue_links(
    #         selector='a.next',
    #         label='CATEGORY',
...
async def detail_handler(context: PlaywrightCrawlingContext) -> None:
    """
    Extracts data from a product/detail page and pushes it to the dataset.
    """
    context.log.info(f'detail_handler is processing {context.request.url}')

    data = {}  # final dict populated in placeholder below

    # {{ EXTRACTION_LOGIC }}
    # Example:
    # title = await context.page.locator('h1.product-title').text_content()
